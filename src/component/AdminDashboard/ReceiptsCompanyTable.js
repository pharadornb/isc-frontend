...
export default function ServiceRecipientsTable() {
  ...
  const handleShow1 = (statement_id) => {
    ...
    axios
      .post("summarize_admin/reciveCompanyById", params, {
        headers: {
    ...
    <>
      <div className="row tb">
        <label className="col-md-8">
          <b className="bm">รายการรับจากบริษัท</b>
        </label>
        ...
        {rows.map((row) => (
          <div key={row.statement_id}>
            <Modal.Header closeButton>
              <Modal.Title>
                {" "}
                <CheckAccuracys sid={row.statement_issuccess} /> {row.uc_name}
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <div className="row">
                ...
                <label className="col-8">
                  <b>ชื่อรายการ : </b>Pay for user_id = {row.statement_id}
                </label>
                <label className="col-8">
