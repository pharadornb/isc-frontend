...
export default function ServiceRecipientsTable() {
    ...
    const handleShow1 = (statement_id) => {
        ...
        axios.post('summarize_admin/reciveCompanyById', params, {
            headers: {
    ...
    const listtable = () => {
        try{
            axios
            .post("summarize_admin/reciveCompany", {
              headers: {
                "Content-Type": "application/json",
              },
            })
    ...
    const CheckStatus = (props) => {
        ...
        if(status === 'yes'){
            return <label className="btn btn-success lc"><b>ผ่านการรับเงิน</b></label>;
        }else if(status === 'no'){
            return <label className="btn btn-danger lc"><b>ไม่ผ่านการรับเงิน</b></label>;
        }else{
    ...
    const CheckAccuracys = (props) => {
        ...
        if(props.sid === 'yes'){
            return(
                <>
                    <img src={CoinWalletImg} alt="logo"></img><b> ได้รายรับจาก:</b> บริษัท 
                </>
            );
        }else if (props.sid === 'no'){
            return(
                <>
                    <img src={NoCoinWalletImg} alt="logo"></img><b> ไม่มีรายรับจาก:</b> บริษัท 
                </>
            )
        }else{
            return('No data!!!')
    ...
    return (
        ...
        <div className="row tb">
                <label className="col-md-8"><b className="bm">รายการรับจากบริษัท</b></label>
                    ...
                    <table className="" aria-label="simple table">
                        ...
                        {data.map((row) => (
                            <tr key={row.uc_name}> 
                                ...
                                <td align="right"><label>{row.statement_price}</label></td>
                                <td align="center"><label><FormatTDate data={row.statement_create}/></label></td>
                                <td align="right">
                                    <button className="btn btn-outline-primary" variant="warning" onClick={() => handleShow1(row.statement_id)}><i className="far fa-check-circle"></i> รายละเอียด</button>
                                </td>
                            </tr>
                        ))}
                    </table>
            ...
            <Modal show={show1} onHide={handleClose1} animation={false} size="lg">
                {rows.map((row) => (
                    ...
                    <Modal.Body>
                        <div className="row">
                            <label className="col-4"><b>ชื่อบริษัท: </b>{row.uc_name}</label>
                            <label className="col-8"><b>ชื่อรายการ : </b>Pay for user_id = {row.statement_id}</label>
                            <label className="col-8"><b>ยอดเงินรับสุทธิ: {row.statement_price} บาท </b></label>
                        </div>
                    </Modal.Body>
                    <Modal.Footer>
