...
export default function ReceiptsCompanyTable() {
    ...
    const handleShow1 = (statement_id) => {
        ...
        axios.post('summarize_admin/reciveUserById', params, {
            headers: {
    ...
    return (
        ...
        <div className="row tb">
                <label className="col-md-8"><b className="bm">รายการรับจากผู้รับริการ</b></label>
            ...
            <Modal show={show1} onHide={handleClose1} animation={false} size="lg">
                {rows.map((row) => (
                    ...
                    <Modal.Body>
                        <div className="row">
                            <label className="col-4"><b>ชื่อ: </b>{row.name}</label>
                            <label className="col-8"><b>รหัสรายการอ้างอิง: </b>{row.statement_code}</label>
                            <label className="col-4"><b>สถานะการโอน: </b> {row.statement_name}</label>
